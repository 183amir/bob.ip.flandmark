...
CExtendedSparseLBPAppearanceModel::CExtendedSparseLBPAppearanceModel(
		int nodeID,
			...
			), kHeightOfPyramid(height_of_pyramid)
{
	...
	kFeatureDimension += size[0]*size[1];
	kSparseFeatureDimension += 1;
...
void CExtendedSparseLBPAppearanceModel::getFeatureAt(const int index, DOUBLE *feature_vector)
{
...
void CExtendedSparseLBPAppearanceModel::computeFeatures(void)
{
	...
	for (int i=0; i < kLength; ++i)
	{
		...
		LBPFeatures[kSparseFeatureDimension*(i+1)-1] = (kSparseFeatureDimension-1)*256 + i;
	}
}

int CExtendedSparseLBPAppearanceModel::getDim(int img_nRows, int img_nCols, int nPyramids)
{
	int w = img_nCols;
	int h = img_nRows;
	int N=0;
...
void CExtendedSparseLBPAppearanceModel::featuresSparse(int *vec)
{
	...
	while (1)
	{
		...
		if (kSparseFeatureDimension-1 <= idx)
			return;
...
void CExtendedSparseLBPAppearanceModel::dotProductWithWq(DOUBLE *const w, DOUBLE *q)
{
	int temp_index = 0;
...
DOUBLE CExtendedSparseLBPAppearanceModel::getQvalue(int *const s_i, DOUBLE *const w)
{
	int	temp_index = 0;
	DOUBLE dotProduct = 0.0;
...
void CExtendedSparseLBPAppearanceModel::write(XmlStorage &fs, DOUBLE * const w, bool writeW) const
{
	fs << "Appearance";
	fs << "{"
	   << "Type" << "EXTENDED_SPARSE_LBP"
