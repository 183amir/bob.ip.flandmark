				...
				base_window_margin_x, base_window_margin_y)
{
	...
	solver = new CTreeMaxSumSolver(&vertices, &edges);
	...
	for (int i=0; i<landmarksCount; ++i)
	{
		...
		{
			q[i].push_back(new DOUBLE[vertices[i].appearances[j]->getFeatureDimension()]);
			w[i].push_back(new DOUBLE[vertices[i].appearances[j]->getFeatureDimension()]);
		}
	}

	for (int i=0; i<edgesCount; ++i)
	{
		g[i] = new DOUBLE[edges[i]->getChild()->getLength()];
		w[kLandmarksCount+i].push_back(new DOUBLE[edges[i]->getDimension()]);
	}
	...
	: CLandmark()
{
	...
	DOUBLE bw_margin_x, bw_margin_y;
...
//	fs["num_nodes"] >> landmarksCount;			// TODO: overload operator >>
	...
	bw_margin_x = (DOUBLE)fs["bw_margin_x"];
	bw_margin_y = (DOUBLE)fs["bw_margin_y"];
...
//					vertices.push_back(vertex);
				...
				case EXTENDED_SPARSE_LBP:
					...
					{
						vertex.appearances.push_back(
									new CExtendedSparseLBPAppearanceModel(
										nodeID,
										vertex.ss[0],
										vertex.ss[1],
										vertex.ss[2],
				...
				break;
				case HOG:
					nbins = (int)it2["n_bins"];
					cell_size = (int)it2["cell_size"];
					block_size = (int)it2["block_size"];
					if (train && (nodeLossType > 0))
					{
						vertex.appearances.push_back(
									new CHOGAppearanceModel(
										nodeID,
										vertex.ss[0],
										vertex.ss[1],
										vertex.ss[2],
			...
			// Allocate and read W
			nodeW = it2["w"];
			w[index].push_back(new DOUBLE[vertices.at(index).appearances[index2]->getFeatureDimension()]);
//			int feature_dimension = vertices.at(index).appearances[index2]->getFeatureDimension();
//			DOUBLE *p_double = new DOUBLE[feature_dimension];
//			w[index].push_back(p_double);
			if (!nodeW.empty())
			{
				int binsize;
				unsigned char *binData = nodeW.readRaw(&binsize);
				DOUBLE *w_ptr = (DOUBLE*)binData;
				// TODO !!! - check if w_ptr is not a null
				memcpy(w[index][index2], w_ptr, sizeof(DOUBLE)*vertices.at(index).appearances[index2]->getFeatureDimension());
				free(binData);
			} else {
				memset(w[index].back(), 0, sizeof(DOUBLE)*vertices.at(index).appearances[index2]->getFeatureDimension());
				// TODO - ^ probably not needed at all
			}

			// Allocate q
			q[index].push_back(new DOUBLE[vertices.at(index).appearances[index2]->getFeatureDimension()]);
			memset(q[index].back(), 0, sizeof(DOUBLE)*vertices.at(index).appearances[index2]->getFeatureDimension());
	...
	for (unsigned int index = 0; index < Edges.size(); ++index)
	{
		...
		// Allocate and read W
		nodeW = it["w"];
		w[kLandmarksCount+index].push_back(new DOUBLE[edges[index]->getDimension()]);
		if (!nodeW.empty())
		{
			int binsize;
			unsigned char *binData = nodeW.readRaw(&binsize);
			DOUBLE *w_ptr = (DOUBLE*)binData;
			// TODO !!! - check if w_ptr is not a null
			memcpy(w[kLandmarksCount+index][0], w_ptr, sizeof(DOUBLE)*edges[index]->getDimension());
			free(binData);
		} else {
			memset(w[kLandmarksCount+index][0], 0, sizeof(DOUBLE)*edges[index]->getDimension());
		}
		// Allocate g
		g[index] = new DOUBLE[edges[index]->getChild()->getLength()];
	}
...
cimg_library::CImg<unsigned char> * Flandmark::getNF(cimg_library::CImg<unsigned char> *img, int * const bbox, DOUBLE * const ground_truth)
{
	if (ground_truth)
	{
		updateScaleFactor(bbox);
...
cimg_library::CImg<unsigned char> * Flandmark::getNFgeneralBB(cimg_library::CImg<unsigned char> *img, int *const bbox, DOUBLE *const ground_truth)
{
	// TODO: check this
	// TODO: propagate angle?
	// TODO: ground_truth!!!
	DOUBLE angle = getNormalizedFrameGeneralBB(img, bbox);

	return normalizedFrame;
}

void Flandmark::setLossTable(DOUBLE *loss_data, const int landmark_id)
{
	for (unsigned int j=0; j < vertices[landmark_id].appearances.size(); ++j)
	{
		DOUBLE *table = new DOUBLE[vertices[landmark_id].appearances[j]->getLength()];
